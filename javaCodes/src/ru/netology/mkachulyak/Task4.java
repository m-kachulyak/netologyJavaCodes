...
public class Task4 {
    public static void main(String[] args) {
        ...
        LocalDate startDate = LocalDate.parse("2024-02-01", DateTimeFormatter.ofPattern("yyyy-MM-dd"));
        LocalDate endDate = LocalDate.parse("2024-02-29", DateTimeFormatter.ofPattern("yyyy-MM-dd"));
        System.out.println("Транзакции в выбранном диапазоне дат (" + startDate + " - " + endDate + "):");

        for (int i = 0; i < 5; i++) {
            if (dates[i].isAfter(startDate) && dates[i].isBefore(endDate) || dates[i].isEqual(startDate) || dates[i].isEqual(endDate)) {
                System.out.println("Транзакция " + (i + 1) + ": " + transactionTypes[i] + ", " + amounts[i] + ", " + transactionIds[i] + ", " + dates[i]);
            }
        }
        scanner.close();
